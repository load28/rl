...
| 자동완성 | match 암 위치의 케이스 태그(이미 덮은 태그 제외 — 대상 enum은 구조적 추론, 실패 시 스크루티니의 **TS 추론 타입**으로 특정), `Enum.` 뒤 생성자(필드 탭스톱 스니펫)**와 그 객체의 TS 멤버**(`Result.map`·`Option.unwrapOrElse` 등 표준 라이브러리 콤비네이터), `Tag(` 안의 필드 바인딩, `enum`/`match`/`try`/`let-else` 스니펫. 그 외 위치·`obj.` 멤버 접근은 TypeScript 언어 서비스의 완성 목록(rl 항목이 위). 항목을 고르면 그 항목의 **타입 시그니처와 JSDoc**을 채워서 보여줌 |
