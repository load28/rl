...
| 기능 | 설명 |
|------|------|
| 문법 하이라이팅 | rl 전용 구문(match 키워드, 암 태그, enum 케이스, `val` 수식자)을 칠하고 나머지는 VSCode 내장 TypeScript 문법(`source.ts`)에 위임 — 통과 원칙과 같은 구조 |
...
- **파싱되는 텍스트일 때만** 검사합니다. 진단에 파스 에러(TypeScript가
  ...
  버립니다 — 그 텍스트는 TypeScript가 아니고, 체커의 오류 복구가 지어낸
  에러가 뒤따르기 때문입니다.
...
참조 찾기·이름 변경·자동완성·시그니처 헬프·타입 진단 — 은 **TypeScript
컴파일러 자신의 언어 서버**(`tsgo --lsp`)가 답합니다. TypeScript 7에는
인프로세스 JS 언어 서비스 API가 없기 때문이며, 확장 프로그램은 TypeScript를
번들하지 않습니다. 탐색 순서:

1. `RLC_TSGO_ROOT` 환경 변수 — 직접 빌드한 typescript-go 체크아웃
   (`built/local/tsgo`)
2. 워크스페이스에서 위로 올라가며 찾는 `node_modules/@typescript/
   typescript-<platform>/lib/tsc` (또는 `native-preview-<platform>`)
3. 서버 프로세스의 디렉터리에서 같은 방식으로 한 번 더

즉 **프로젝트가 `typescript@7`을 설치해 두면 그대로 동작합니다.** 찾지
못하면 위 위임 기능들이 답하지 않고, rl 자신이 아는 것(enum·케이스 태그·
소진성)만 동작합니다.

표준 라이브러리(`@rl/std`)는 모듈 해석이 디스크를 보므로, 그것을 import한
파일을 열면 확장 프로그램이 워크스페이스의 `node_modules/@rl/std`에
`rlc --emit-std` 결과를 한 번 써 둡니다 (이미 있는 패키지는 건드리지
않습니다).
