...
| 자동완성 | match 암 위치의 케이스 태그(이미 덮은 태그 제외 — 대상 enum은 구조적 추론, 실패 시 스크루티니의 **TS 추론 타입**으로 특정), `Enum.` 뒤 생성자(필드 탭스톱 스니펫), `Tag(` 안의 필드 바인딩, `enum`/`match`/`try`/`let-else` 스니펫. 그 외 위치·`obj.` 멤버 접근은 TypeScript 언어 서비스의 완성 목록(rl 항목이 위) |
...
`try`/`let-else`/`if let` 식·파이프라인 스텝 *내부*에서도 호버·완성·정의
이동이 온전한 타입 추론으로 동작합니다. match 암 자동완성은 스크루티니의
TS 추론 타입으로 대상 enum을 특정합니다(구조적 추론이 실패할 때).
컴파일러가 없거나 방출이 아직 따라오지 못한 순간에는 원문을 그대로
서빙하는 예전 방식(TS 오류 복구)으로 자동 폴백합니다. TS 쪽 **진단은
표출하지 않습니다** — 에러는 언제나 rlc가 정본입니다.
