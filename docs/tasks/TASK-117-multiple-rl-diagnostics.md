...
> 격차를 나중에 처음 보는 사람이 재현·판단할 수 있도록 남긴다.
>
> **구현은 [TASK-120](./TASK-120-structured-diagnostics.md)이 했다** —
> 증상 1(하나만 보고)·증상 2(문안 비대칭)·증상 3(rl 에러가 typed 진단을
> 가림) 전부. 설계 쟁점들의 결론은 TASK-120의 의사결정 절에 있다: 쟁점 1은
> match 단위 복구 경계, 쟁점 2는 `compile()` 유지 + `analyze()`/
> `compile_report()`, 쟁점 3은 소스 순서, 쟁점 4는
> `DiagnosticCode::blocks_projection`으로 `Blocked` 완화.
