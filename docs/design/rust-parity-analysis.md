...
`(7,57)`은 방출된 `return $rl_t0;` — **순수 글루**다. 사용자는 자기가 쓰지 않은
줄에서, 자기가 쓰지 않은 이름(`$rl_t0`)으로 에러를 받는다. rl이 여기서 지고
있는 것은 두 개다: **위치**와 **문안**. 타입 자체는 tsc가 이미 정확히 알고
있다 — rl이 못 하는 것은 "타입을 아는 것"이 아니라 **"아는 답을 자기 좌표와
...
rlc는 자기가 만든 글루가 **어느 구성물에서 나왔는지 정확히 안다.** 그러니
글루 조각마다 **단방향 진단 앵커**(글루 span → 그 구성물의 소스 span)를 함께
방출할 수 있다. `return $rl_t0;` → `try` 키워드, `$rl_m.kind` switch →
`match` 키워드, `$rl_ap(...)` → 해당 스텝.
...
> **상태: 부분 구현** (TASK-108) — `ask::Members`에 해당하는 첫 질문이
> 들어갔다. 체커가 스크루티니 타입의 구성원 목록을 답하고, 소진성 계산은
> 기본 경로와 같은 usefulness 알고리즘이 한다. 남은 것은 **중첩 열**의 같은
> 질문(`Ok(value: ...)`의 `value` 자리)과 `Discriminant`/`Property`/`Display`다.
...
계약 영향: **없음.** rlc는 여전히 타입을 계산하지 않는다 — `val`과 똑같이 묻고
...
rlc: f.rl:2:13: `try`가 전파하는 Err 타입 `string`이 함수의 반환 타입
     `Result<number, number>`와 맞지 않습니다 (rl에는 Rust의 `From` 같은
     자동 변환이 없습니다 — 반환 타입을 넓히거나 에러를 변환하세요)         ← 번역 후
```

**이것은 계약 위반이 아니라 계약의 이행이다.** 계약 2가 요구하는 상태는
"사용자가 rlc의 코드 때문에 TS 에러를 마주하지 않는 것"이고, 지금은 그 상태에
...
- 번역된 진단에는 원문을 `note:`로 붙인다. rl 문안이 틀렸을 때 사용자가
  스스로 판단할 수 있어야 한다.
...
> **검사 전용 프로젝션**: rlc가 체커에게 보내는 텍스트에만 존재하고, 사용자가
> 받는 산출물에는 존재하지 않는 코드.
...
소진성의 `never` 가드다(`const $rl_x: never = $rl_m;`). 다만 소진성은 계층 2로
...
> (현행) 생성되는 코드는 타입 트릭 없는 순수 TypeScript이며, rlc가 방출한 코드
> 때문에 tsc 에러가 발생해서는 안 된다.
>
> (개정안) **사용자가 받는 산출물**은 타입 트릭 없는 순수 TypeScript다. 검사
> 시점에만 존재하는 프로젝션은 검사를 위한 구성물을 포함할 수 있으나, 거기서
> 나온 진단은 **반드시 rlc가 rl의 문안과 위치로 번역해서** 보고한다. 번역되지
> 않은 TS 진단이 rlc의 코드에서 사용자에게 도달하면 버그다.
...
- **산출물에 타입 트릭 심기.** 예: 리터럴 match의 `default`에 `never` 가드를
  ...
  "rlc 없이 tsc만 돌리는 CI에서도 소진성이 잡힌다" 하나뿐인데, 그건 CI에
  `rlc --check-types`를 넣으면 되는 문제다. 잃는 것은 에러 계층 전체(생성물
  좌표·TS 문안)와, 생성된 `.ts`를 읽는 사람이 마주할 정체불명의 코드다.
- **rlc가 타입을 조금 아는 것(부분 타입 시스템).** 선언 타입 텍스트를 파싱해
...
2. **계층 2(seam 확장)** — §9.3이 못 한다고 한 것의 절반 이상이 여기서 닫힌다.
   P2·P4가 이 위에 선다.
3. **계층 3(진단 번역)** — 문안까지 rl의 것으로. 계약 2가 의도한 상태에 실제로
   도달한다.
