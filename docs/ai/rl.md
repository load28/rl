...
- Assignability errors are rendered from checker facts, not by rewriting TypeScript prose: ``type mismatch: expected `InputError`, found `RangeError` `` plus ``required type: `Result<number, InputError>` `` when the surrounding obligation helps. The same syntax-neutral rule covers return values, annotations, call arguments, and lowered RL constructs. A lowered construct with one proven mismatch reports that cause once and suppresses property/comparison consequences from the same glue anchor. RL enum cases use declaration names when uniquely identifiable; otherwise checker structural names remain. CLI, server, and the editor consume this same message and `ts<code>` identity. Non-assignability diagnostics still use the construct-specific translation table or raw checker fallback.
