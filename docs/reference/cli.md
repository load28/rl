...
| `--types` | 빌드 대신 **타입 사이드카**를 만듭니다: 선언을 **메모리에서** 방출해 `<이름>.rl.d.ts`(+`.map`)를 `-o`(기본 `.rl-types/`) 아래에 씁니다. 중간 트리를 만들지 않습니다 (아래 "타입 생성"). |
| `--node <path>` | `--types`가 선언 방출 호스트를 실행할 node 바이너리 (기본: PATH의 `node`). TypeScript는 **프로젝트의 `node_modules`**에서 해석합니다. |
...
- **디렉터리**면 재귀적으로 순회하며 **`.rl` 파일과, 컴파일 계열
  ...
  정렬 순서로 처리됩니다.
- 이름이 `.`으로 시작하는 디렉터리(`.git`, `.rl-types`, ...)와
...
1. 각 `.rl`을 **메모리에서** 컴파일합니다. 결과는 그 파일이 디스크에 있었다면
   차지할 경로(`src/engine.rl` → `src/engine.ts`)의 가상 모듈이 됩니다.
   실제로 그 이름의 파일이 있으면 거부합니다.
2. 내장된 호스트 스크립트를 `node`로 실행해 TypeScript API로 선언을
   방출합니다. **손으로 쓴 `.ts`는 전달하지 않습니다** — 호스트가 디스크에서
   제자리에서 읽으므로 어떤 소스도 복사되지 않습니다. 그 파일들도 프로그램에
   경로로 참여하므로 그쪽의 타입 에러도 그대로 보고됩니다.
   `.rl` 지정자와 `@rl/std`는 호스트가 직접 해석합니다 — 선언 심이나 합성
   tsconfig가 필요 없습니다.
3. 각 `.rl`의 선언을 에디터 사이드카(`<이름>.rl.d.ts` + `.map`)로 바꿔
   `-o`(기본 `.rl-types/`) 아래에 입력 구조를 미러하며 씁니다.
   `@rl/std`를 쓰면 그 선언도 `rl.d.ts`로 함께 나옵니다.

**디스크에 남는 것은 사이드카뿐입니다.** TypeScript는 프로젝트의
`node_modules`에서 해석하고(없으면
`rlc: typescript not found — install it (npm i -D typescript)`), 타입 에러가
있어도 선언은 방출되므로 사이드카는 갱신되고 종료 코드만 1이 됩니다.
