...
- `declarations { path, text }`는 버퍼에서 **보이는** enum 선언 표 —
  로컬(스팬 포함)·import(별칭 적용, `specifier`)·내장(`Option<T>`/
  `Result<T, E>`) — 를 컴파일러의 shadowing 그대로 답하고, 버퍼의
  `match` 위치(`keyword`)와 암 삽입 지점(`bodyClose` — 본문 닫는 `}`)을
  함께 답합니다. 좌표는 바이트 오프셋입니다. 에디터의 자체 rl 구문
  해석을 대체하는 표면입니다.
