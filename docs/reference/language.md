...
중복 암 규칙이 잡지 못하는 죽은 암(중첩 패턴이나 튜플 조합 때문에 앞선 암이
이미 전부 잡는 경우)은 **에러가 아닙니다** — 에디터가 `rlHints`로 받아
흐리게 표시합니다([`cli.md`](./cli.md#엔진-서버---server)).
...
- 검사 대상 enum은 사이트가 정합니다: `match`는 암들의 태그를 가장 많이 포함하는
  유일한 enum, let-else·`if let`은 태그가 **하나**뿐이면 **편집 한 번** 거리의
  케이스를 가진 enum이 유일할 때만이고, or-패턴의 여러 태그는 match와 같은
  근거라 match의 규칙을 그대로 씁니다. 중첩 패턴은 바깥 필드의 선언된
  타입입니다.
...
`try`는 **함수 본문의 문장 위치**에서만 씁니다. 판정은 제어 흐름
기반입니다: 방출되는 `return`이 나갈 **사용자가 그 자리에 쓴 함수**가
있는가 — Rust의 클로저 안 `?`처럼, rl 구성물 안이라도 함수 안이면
됩니다.

| 위치 | 결과 |
|------|------|
| 함수·메서드·화살표 함수 본문의 문장 위치 (제어문 블록 포함) | 허용 |
| match 내부·템플릿 보간·파이프라인 스텝·`result` 블록 **안에 쓴 함수의 본문** | 허용 — `return`은 그 함수를 벗어납니다 (`match (x) { A(v) if run(() => { try g(); ... }) => ... }`) |
| `if let` 본문·let-else `else` 블록 (**인라인** 문맥) | 그 문 자체가 함수 안이면 허용 — 인라인 블록의 문장은 문이 선 자리에서 실행되므로 `return`이 같은 함수를 벗어납니다. 문이 match 암 등 IIFE 영역에 있으면 그 체인의 `try`도 에러 |
| `return try f()`·화살표 함수 본문·삼항식 같은 표현식 위치 | 컴파일 에러 — `try`는 표현식이 아니라 문장입니다. 먼저 `const value = try f();`로 바인딩한 뒤 그 값을 반환하거나 사용하세요 |
| match 내부(스크루티니·암 본문)·`result` 블록·템플릿 보간의 문장 위치 (함수 없이 직접) | 컴파일 에러 — 그 자리의 `return`은 둘러싼 함수가 아니라 구성물의 IIFE에서 반환됩니다. 함수로 추출하거나 `result` 블록의 `<-` 바인딩을 쓰세요 |
| 모듈 최상위, namespace 본문 등 함수 아닌 스코프 | 컴파일 에러 — 방출되는 `return`이 나갈 함수가 없습니다 |
...
패턴        ::= 태그 "(" 바인딩-목록? ")" ("|" 대안)*   // 첫 대안은 괄호 필수
대안        ::= 태그 ("(" 바인딩-목록? ")")?             // 이후 대안은 태그만도 가능
블록        ::= "{" 문장* "}"
```

**첫 대안의 괄호와 세미콜론이 필수**입니다 — 없으면 rl 구문이 아니고
통과합니다. 일반 TypeScript 선언과 `if/else`는 전부 통과합니다(유효한 TS에서
선언 키워드 뒤에 `식별자(`가 오는 일은 없습니다). or-패턴은 match와 같은
규칙입니다: 모든 대안이 같은 (필드, 이름) 집합을 바인딩해야 하고(구조 분해를
공유하므로), 바인딩은 별칭까지만 — 중첩 패턴은 없습니다. `||`는 구분자가
아닙니다.

### 6.2 의미

Rust의 `let ... else`입니다. 식을 한 번만 평가해 `kind`가 대안 중 한 태그와
맞으면 필드를 풀고, 아니면 `else` 블록을 실행합니다. `else` 블록은 **반드시
발산**해야 합니다.
...
- 사용 위치 제약은 `try`와 같은 제어 흐름 판정입니다
  ([§5.4](#54-사용-위치-제약)) — 단, **모듈 최상위는 허용**됩니다: let-else
  자체는 `return`을 방출하지 않으므로(`throw`로 발산하는 `else`는 어디서나
  유효), rl 구성물의 문장 영역(match 암 본문, `result` 블록, 템플릿 보간)
  안에서만 그 자리에 쓴 함수가 필요합니다.
- 발산 검사는 **제어 흐름 검사**입니다: else 블록의 모든 경로가
  `return`/`throw`/`break`/`continue`로 블록을 벗어나야 합니다. 발산
  문장으로 끝나는 블록은 물론, `if (c) return a; else return b;`처럼
  모든 분기가 발산하는 `if`/`else`(체인 포함), 발산하는 중첩 블록
  `{ return a; }`, 발산 문장 뒤의 도달 불가 코드도 인정됩니다. 판정이
  모델링하지 않는 구조(루프, `switch`, `try`)는 보수적으로 비발산으로
  칩니다 — 오탐으로 통과시키는 일은 없습니다. else 블록 안에 선언된
  함수의 `return`은 그 함수를 벗어날 뿐이므로 인정되지 않습니다.
...
```
if-let-문 ::= "if" "let" 태그 "(" 바인딩-목록? ")" ("|" 대안)* "=" 식 블록
              ("else" (블록 | if-let-문))?
대안      ::= 태그 ("(" 바인딩-목록? ")")?
```
...
- 바인딩 문법은 match 패턴과 같습니다: **중첩 패턴**
  (`if let Ok(value: Some(value: v)) = r { ... }`)과 **or-패턴**
  (`if let Circle(r) | Square(r) = s { ... }`)이 되고, match처럼 둘을
  조합할 수는 없으며 or-대안들은 같은 (필드, 이름) 집합을 바인딩해야
  합니다.
...
있고, 표현식 위치(템플릿 보간, 스크루티니, 표현식 암 본문 등)에 직접 쓰면
컴파일 에러입니다 — 단, 그 자리에 **쓴 함수의 본문 안**은 문장 위치이므로
허용됩니다(`try`와 같은 제어 흐름 판정, [§5.4](#54-사용-위치-제약)).
...
| 빈 스텝 | `x \|>;`, `x \|> \|> f` — 컴파일 에러 |
| head/step의 문장 위치에 직접 쓴 `try` | match와 같은 사유로 금지 — 스텝 **안에 쓴 함수의 본문**에서는 허용 ([§5.4](#54-사용-위치-제약)) |
...
| 선언 키워드 없는 바인딩 (`b <- f();`) | 그 블록이 이미 rl로 판별됐거나, `result {`가 TypeScript일 수 없는 자리(식이 시작하는 자리 + 같은 줄의 `{`)일 때만 위치를 담은 rl 에러. 그 밖에는 통과 (`b < -f()` 비교) |
| 블록 최상위 **아래**의 바인딩 (`if (c) { const x <- f(); }`, 블록 안에 쓴 함수 본문의 `<-`) | 위치를 담은 rl 에러 — 바인딩은 블록 IIFE의 early return으로 컴파일되므로 최상위 문장만 될 수 있습니다. 최상위로 끌어올리거나 `match`를 쓰세요. (선언 키워드 뒤의 `<-`는 어디서든 TypeScript일 수 없어 통과 계약도 깨지 않습니다; `let x: Foo<-1>;`은 중첩 깊이에서도 통과) |

선언 키워드 뒤의 `<-`는 유효한 TypeScript일 수 없으므로(선언자에는 초기화
`=`가 필요합니다) 이 판별은 통과 계약을 깨지 않습니다. 같은 이유로, 바인딩이
있는데 파싱에 실패한 블록은 통과시킬 수 없어 **에러**가 됩니다.

키워드를 빠뜨린 바인딩(`b <- f();`)은 그 자체로는 유효한 TypeScript 비교
(`b < -f()`)이므로, **그 텍스트가 TypeScript일 수 없다는 것이 이미 확정된
곳에서만** 에러로 보고합니다: ① 그 블록의 다른 문장이 이미 진짜 바인딩이라
블록이 rl로 판별됐거나, ② `result {`가 식이 시작하는 자리에 있고 `{`가
`result`와 같은 줄에 있을 때(그 형태는 TypeScript가 될 수 없습니다 —
`function f(): result {`는 `:` 뒤라서, `type X = result` + 다음 줄 블록은
줄바꿈이 있어서 각각 제외됩니다). 비교를 쓰려던 것이라면 `<`와 `-` 사이에
공백을 둡니다.
...
- 블록 안의 `return`은 **블록에서** 빠져나옵니다(둘러싼 함수가 아니라).
  그래서 블록의 문장 위치에 직접 쓴 `try` 문과 let-else는 에러입니다 — 둘 다
  둘러싼 함수에서 나가는 `return`으로 컴파일되기 때문입니다. 블록 안에 쓴
  함수의 본문에서는 허용됩니다 ([§5.4](#54-사용-위치-제약)). `if let`은
  자체 완결 블록으로 컴파일되므로 자유롭게 씁니다.
...
| 도달 불가 암 | 무가드 암이 이미 덮은 태그의 반복(중복 암)만 **에러**입니다. 중첩·조합까지 따져 찾은 죽은 암은 컴파일을 막지 않고 **에디터 힌트**로만 나옵니다(`rlHints`, [`cli.md`](./cli.md)) — rl에는 경고 계층이 없어 에러로 만들면 지금 컴파일되는 프로그램이 깨집니다 |
...
| `let-else` | 패턴 괄호와 `;` 필수, else의 모든 경로가 발산해야 함. or-패턴·가드·중첩 패턴, `= try 식 else` 조합 미지원 |
